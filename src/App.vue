...
import SidebarChatsMobile from './components/SidebarChatsMobile.vue';
...
const forwardCommentText = ref('');

const mobilePane = ref<'list' | 'chat' | 'info'>('list');

const showMobileChatsList = computed(() => !isMobile.value || mobilePane.value === 'list');

const showMobileChatView = computed(() => !isMobile.value || mobilePane.value === 'chat');
const showMobileInfoLayer = computed(() => isMobile.value && mobilePane.value === 'info');
...
const mobileHeaderPresenceText = computed(() => {

  if (!activeConversation.value) return '';



  if (mobileHeaderIsTyping.value) {
  ...
  if (activeConversation.value.type === 'direct') {
    ...
    return mobileHeaderIsOnline.value ? 'онлайн' : 'не в сети';

  }



  if (activeConversation.value.type === 'group') return 'Группа';
...
const mobileHeaderIsOnline = computed(() => {

  if (activeConversation.value?.type !== 'direct') return false;

  return !!directChatProfileUser.value?.isOnline;

});



const mobileHeaderIsTyping = computed(() => {

  return activeConversation.value?.type === 'direct' && !!typingText.value.trim();
...
watch(
  ...
  ([mobile, conversationId]) => {
    ...
    mobilePane.value = conversationId ? 'chat' : 'list';
...
function closeTopOverlay() {
  ...
  if (isMobile.value && activeConversationId.value) {
    mobilePane.value = 'chat';
  }
}
...
async function handleAttachPicked(payload: {
  ...
  if (!activeConversationId.value || !payload?.files?.length) return;
...
function handleReplyToSpecificAttachment(payload: { message: Message; attachmentKey: string }) {
  replyToMessage.value = payload.message;
  replyToAttachmentKey.value = payload.attachmentKey;
}
...
async function handleDeleteSpecificAttachment(payload: {
  message: Message;
  ...
  await api.post(`/conversations/${activeConversationId.value}/attachments/delete`, {
  ...
  messages.value = messages.value
    .map((message) => {
      if (message.id !== payload.message.id) return message;

      const nextAttachments = (message.attachments || []).filter(
        (attachment) =>
          attachment.id !== payload.attachmentKey &&
          attachment.tempId !== payload.attachmentKey &&
          attachment.relativeUrl !== payload.attachmentKey
      );

      if (nextAttachments.length === 0 && !message.text.trim()) {
        if (payload.mode === 'self') return null;
        return {
          ...message,
          text: 'Сообщение удалено',
          replyTo: null,
...
function handleForwardSingleMessage(payload: { message: Message; attachmentKey: string }) {
  if (!payload?.message?.id || !activeConversationId.value) return;
  forwardAttachmentPayload.value = payload;
  errorText.value = '';
  forwardSearchQuery.value = '';
  isForwardModalOpen.value = true;
}

function handleReportMedia(message: Message) {
  const username = message?.username || 'пользователь';
  errorText.value = `Жалоба на медиа от ${username} пока недоступна`;
}
...
async function submitForwardSelectedMessages(targetConversationId: string) {
  ...
  try {
    if (forwardAttachmentPayload.value) {
      await api.post(`/conversations/${sourceConversationId}/attachments/forward`, {
        targetConversationId,
        messageId: forwardAttachmentPayload.value.message.id,
        attachmentKey: forwardAttachmentPayload.value.attachmentKey,
      });

      forwardAttachmentPayload.value = null;
      isForwardModalOpen.value = false;
      forwardSearchQuery.value = '';
    ...
    if (forwardMessageIds.value.length === 0) return;
...
function emitTypingStart() {
  ...
  if (!socket || !activeConversationId.value || !socket.connected) return;
  ...
  typingTimeout = window.setTimeout(() => {
    ...
    if (!currentSocket || !activeConversationId.value) return;



    currentSocket.emit('typing:stop', { conversationId: activeConversationId.value });
...
<template>
  <AppMobileLayout v-if="isMobile" :mode="mode">
<div style="height: 100%; display: flex; flex-direction: column; min-height: 0; overflow: hidden;">
    ...
    <template v-else>
      <div class="tm-shell tm-shell--wide">
        <SidebarChatsMobile
          ...
          v-if="isMobile && showMobileChatsList"
        ...
        <MobileChatHeader
          ...
          v-if="isMobile && activeConversationId && mobilePane === 'chat'"
          ...
          :presence-text="mobileHeaderPresenceText"
          :is-online="mobileHeaderIsOnline"
          :is-typing="mobileHeaderIsTyping"
        />



        <ChatView
          v-if="showMobileChatView && !showMobileInfoLayer"
        ...
        <div
          ...
          v-if="showMobileInfoLayer"

          class="tm-mobile-info-layer"
        ...
        >
          ...
          <div class="tm-mobile-info-layer__body">

            <div class="tm-mobile-info-card">

              <div class="tm-mobile-info-card__label">Название</div>

              <div class="tm-mobile-info-card__value">

                {{ activeConversation?.title || 'Без названия' }}
            ...
            <div class="tm-mobile-info-card">
              ...
              <div class="tm-mobile-info-card__label">Тип</div>
            ...
            <div class="tm-mobile-info-card">
              ...
              <div class="tm-mobile-info-card__label">ID чата</div>

              <div class="tm-mobile-info-card__value tm-mobile-info-card__value--mono">
      ...
      <ForwardMessagesModal
        ...
        :comment-text="forwardCommentText"
        :active-conversation-id="activeConversationId"
        @close="isForwardModalOpen = false; forwardSearchQuery = ''; forwardCommentText = ''; forwardMessageIds = []"
        @update-search-query="forwardSearchQuery = $event"
        @update-comment-text="forwardCommentText = $event"
  ...
  <AppDesktopLayout v-else :mode="mode">
...
.tm-mobile-back-btn {
  ...
  border-bottom: 1px solid rgba(100, 140, 210, 0.16);
...
.tm-mobile-info-layer {
  ...
  background:
    ...
    radial-gradient(circle at top, rgba(110, 170, 255, 0.18), transparent 26%),
...
.tm-mobile-info-card {
  ...
  border: 1px solid rgba(117, 162, 231, 0.14);

  box-shadow: 0 12px 28px rgba(56, 88, 140, 0.08);

}



.tm-mobile-info-card__label {
...
.tm-mobile-info-card__value--mono {
